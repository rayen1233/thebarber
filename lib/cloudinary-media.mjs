...
/** Cloud name correct pour The Barber (corrige dsdswnmoq8d sur Vercel). */
export const CANONICAL_CLOUDINARY_CLOUD_NAME = "dswnmoq8d";

/** @param {string} raw */
export function normalizeCloudName(raw) {
  let v = String(raw || "")
    .trim()
    .replace(/[\s\r\n\\]+/g, "")
    .toLowerCase();
  if (v.includes(CANONICAL_CLOUDINARY_CLOUD_NAME) && v !== CANONICAL_CLOUDINARY_CLOUD_NAME) {
    return CANONICAL_CLOUDINARY_CLOUD_NAME;
  }
  return v;
}

/** @param {string} url */
export function fixCloudinaryAssetUrl(url) {
  const v = String(url || "").trim();
  if (!/res\.cloudinary\.com/i.test(v)) return v;
  return v.replace(
    /res\.cloudinary\.com\/[a-z0-9]+/i,
    `res.cloudinary.com/${CANONICAL_CLOUDINARY_CLOUD_NAME}`,
  );
}
...
export function deliveryVideoUrl(publicId) {
  ...
  return cloudinary.url(publicId, {
    ...
    secure: true,
    transformation: [
      { quality: "auto:good", fetch_format: "mp4", width: 1280, height: 720, crop: "limit" },
    ],
  });
}
...
export function getAllShowroomCloudinaryDelivery() {
  return [0, 1, 2, 3]
    .map((i) => {
      const row = getShowroomCloudinaryDelivery(i);
      if (!row) return null;
      return {
        ...row,
        videoUrl: fixCloudinaryAssetUrl(row.videoUrl),
        posterUrl: fixCloudinaryAssetUrl(row.posterUrl),
      };
    })
    .filter(Boolean);
}
