...
export async function loadStore() {
  ...
  try {
    let best = null;
    let bestScore = -1;
    for (const pathname of [BLOB_GZ, BLOB_JSON]) {
      const store = await loadFromBlobPathname(pathname);
      if (!store) continue;
      const score = storeRichness(store);
      if (score > bestScore) {
    ...
    if (listed && storeRichness(listed) > bestScore) {
      best = listed;
      bestScore = storeRichness(listed);
    }
    if (best) return best;
    return defaultStore();
  } catch (err) {
...
export async function mergeStore(partial) {
  ...
  if (Array.isArray(partial.products) && partial.products.length) {
    for (const p of partial.products) {
      ...
      const id = String(/** @type {{ id?: string }} */ (p).id || "");
  ...
  if (Array.isArray(partial.users) && partial.users.length) {
    current.users = partial.users;
  }
  if (Array.isArray(partial.orders) && partial.orders.length) {
    current.orders = partial.orders;
  }
