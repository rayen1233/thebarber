...
/**
 * URL à stocker dans le catalogue (chemin relatif /api/media, pas localhost absolu).
 * @param {string} raw
 */
export function normalizeVideoUrlForCatalog(raw) {
  const v = String(raw || "").trim();
  if (!v) return "";
  if (v.startsWith("idb://") || v.startsWith("data:")) return v;
  if (v.startsWith("/api/media?")) return v;
  if (/^https?:\/\//i.test(v)) {
    if (isPrivateBlobUrl(v)) {
      const pathname = pathnameFromBlobUrl(v);
      if (pathname) return mediaProxyUrl(pathname);
    }
    try {
      const u = new URL(v);
